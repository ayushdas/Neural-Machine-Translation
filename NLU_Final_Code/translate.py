...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        with torch.no_grad():
            ...
            encoder_out = model.encoder(sample['src_tokens'].cuda(), sample['src_lengths'].cuda())
            go_slice = \
                torch.ones(sample['src_tokens'].cuda().shape[0], 1).fill_(tgt_dict.eos_idx).type_as(sample['src_tokens'].cuda())
            prev_words = go_slice
            next_words = None
