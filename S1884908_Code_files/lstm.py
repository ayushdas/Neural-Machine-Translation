...
class LSTMEncoder(Seq2SeqEncoder):
   ...
   def forward(self, src_tokens, src_lengths):
       ...
       '''
       '''
           When self.bidirectional is set to True, it makes the network behave as a bidirectional LSTM encoder.
       This effect is achieved by concatenating the final hidden states and the final cell states in each direction.
           In LSTM, the output of LSTM cell is called the Hidden state and internal state of a cell is called the cell state.
       The final_hidden_states are obtained by using the outputs from the hidden layers, whereas final_cell_states exist for
       each of the cells in the layer.
       '''
...
class AttentionLayer(nn.Module):
   ...
   def forward(self, tgt_input, encoder_out, src_mask):
       ...
       '''
       '''
           The attention context vector is calucated by the batch matrix-matrix multiplication (dot product)
       of the attention weights (obtained by normalizing the attention scores) and encoder outputs.
           Masking needs to be applied to handle attention scores of sentences of various lengths. Setting the padded values
       of the attention scores to -inf and applying softmax, causes the attention weights to focus only on the valid symbols
       in then sentence and assign ~zero weights to pad symbols.
       '''
   ...
   def score(self, tgt_input, encoder_out):
       ...
       '''
       '''
           The attention scores are computed by batch matrix-matrix multiplication (dot product) of the
       projected_encoder_output and the corresponding targets of the batch inputs. This dot product is done
       using the pytorch function torch.bmm(). The projected_encoder_output
       is computed by applying src_projection to the encoder_ouputs in order to make the dimensions consistent
       for the dot product with the target inputs.
           The result of the matrix mutiplication enables the system, to obatin attention on the appropriate word.
       '''
...
class LSTMDecoder(Seq2SeqDecoder):
   ...
   def __init__(self,
       ...
       # self.use_lexical_model = True
       if self.use_lexical_model:
           # __QUESTION: Add parts of decoder architecture corresponding to the LEXICAL MODEL here
           self.layer_1 = nn.Linear(embed_dim, embed_dim, bias=False)
           self.transform_1 = nn.Linear(embed_dim, len(dictionary),bias = True)
           self.transform_2 = nn.Linear(hidden_size, len(dictionary), bias = True)
           # TODO: --------------------------------------------------------------------- /CUT
   ...
   def forward(self, tgt_inputs, encoder_out, incremental_state=None):
       ...
       first word in a sentence.
           input_feed is the output of vector from the attention layer vectors. It passes within the LSTM, context information
       ...
       for j in range(tgt_time_steps):
           ...
           The Attention step weights are updated through timesteps and are stored in step_attn_weights.   
               The attention function is given the previous target state as one of its inputs because this enables information to pass
           ...
           and recurrent connections to LSTM units are excluded through a probability, during the training process.
           '''
           if self.attention is None:
               ...
               if self.use_lexical_model:
                   ...
                   step_attention_reshaped = step_attn_weights.view(batch_size,1,step_length)
                   f_t = F.tanh(torch.bmm(step_attention_reshaped,src_embeddings_transpose))
                   h_t = F.tanh(self.layer_1(f_t)) + f_t
                   y_t = self.transform_1(h_t) + self.transform_2(input_feed.view(batch_size,1,-1))
                   lexical_contexts.append(y_t) # Softmax is computed in the cross etropy loss
                   # TODO: --------------------------------------------------------------------- /CUT
       ...
       if self.use_lexical_model:
           ...
           decoder_output = torch.cat(lexical_contexts, dim=0).view(batch_size, tgt_time_steps, -1)
           # TODO: --------------------------------------------------------------------- /CUT
